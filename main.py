...
@app.post("/query/{user_id}/{session_id}")
async def ask_query(user_id: str, session_id: str, query: Query, namespace: str, session: SessionContainer = Depends(verify_session())):
    existing_sessions = load_sessions(user_id)
    if not any(session["session_id"] == session_id for session in existing_sessions):
        raise HTTPException(status_code=404, detail="Session not found")
    ...
    async def openai_response_stream():
        response = client.chat.completions.create(
            model="gpt-4",
        ...
        for chunk in response:
            ...
            await asyncio.sleep(0.1)  # Adding a small delay to ensure the client can process the stream
